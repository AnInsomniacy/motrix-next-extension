...
describe('DuplicateDownloadGuard', () => {
  it('blocks a repeated URL inside the configured window and notifies once', () => {
    ...
    const first = guard.reserve(input, settings);
    expect(first.blocked).toBe(false);

    const second = guard.reserve(input, settings);
    expect(second).toEqual({ blocked: true, shouldNotify: true });

    const third = guard.reserve(input, settings);
    expect(third).toEqual({ blocked: true, shouldNotify: false });

    now += 10_001;
    expect(guard.reserve(input, settings).blocked).toBe(false);
  });
  ...
  it('does not block when disabled', () => {
    ...
    const first = guard.reserve(input, disabled);
    const second = guard.reserve(input, disabled);
